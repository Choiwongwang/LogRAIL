...
for name in ("postprocess.prompts", "postprocess.prompts_Normal", "prompts_Normal"):
    try:
        prompts_mod = importlib.import_module(name)
        break
    except Exception:
        continue
if prompts_mod is None:
    raise ImportError("Failed to import prompts (tried postprocess.prompts, postprocess.prompts_Normal)")
...
DEFAULTS = {
    ...
    "normal_csv": "dataset/normal_templates_clean.csv",
...
class RAGPostProcessorV2:
    ...
    def _select_evidence_pairs_normal(self, vdb: Chroma, templates: List[str]) -> List[dict]:
        """Bottom-k by similarity against Normal VDB (low sim = suspicious)."""
        pairs: List[dict] = []
        for tpl in templates:
            t = str(tpl).strip()
            if not t or t == self.pad_token:
                continue
            hits = self._retrieve(vdb, t, k=1)
            if hits:
                ctx, sim = hits[0]
            else:
                ctx, sim = "", 0.0
        ...
        if not pairs:
            return []
        pairs.sort(key=lambda d: d["sim"])  # bottom-k
        return pairs[: min(self.evidence_k, len(pairs))]

    # ---- Chat prompt (pairs) ----
    def _build_chat_pairs(self, pairs: List[dict], strict: bool = False) -> str:
        ...
        for i, p in enumerate(pairs, 1):
            lines.append(
                ...
                f"- Top-1 normal template: {p.get('ctx','')}\n"
                f"- Similarity: {p.get('sim',0.0):.3f}"
            )
        pairs_block = "\n\n".join(lines) if lines else "(no evidence pairs)"

        user = (
            "Decide anomaly (1) vs normal (0) for the WINDOW based on evidence pairs.\n\n"
            f"{pairs_block}\n\n"
    ...
    def run(
        ...
        for r in it:
            ...
            if do_call and tpls:
                pairs = self._select_evidence_pairs_normal(vdb, tpls)
                if pairs:
                    # Use the most suspicious pair (lowest sim) for display
                    tpl = pairs[0]["tpl"]
                    ctx_top1 = pairs[0]["ctx"]
                    sim_top1 = float(pairs[0]["sim"])

                    strict_mode = (base_pred == 1)
                    prompt_text = self._build_chat_pairs(pairs, strict=strict_mode)
                    js = self._run_llm(prompt_text)
                    called += 1
                    if isinstance(js, dict):
                    ...
                    if llm_vote in (0, 1):
                        ...
                        final_pred = base_pred
                        guard_note = "ParseError(llm_pairs)"
        ...
        print(f"[i] call_policy={call_policy}  LLM-called: {called}")
